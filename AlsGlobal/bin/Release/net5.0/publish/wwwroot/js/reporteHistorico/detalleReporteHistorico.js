var detalleReporteHistorico = function () {
    ...
    var inicializarEvento = function () {
        ...
        $("#id_proyecto").on("change", obtenerEstacionesConProyecto);
    ...
    var obtenerEstaciones = function () {
        ...
        $.post(urlObtenerEstacionesProyectosYLimites, model)
            .then(response => {
                $("#id_estacion, #id_limite, #id_proyecto, #id_parametro").empty();
                ...
                response.proyectos.forEach(item => {
                    var option = document.createElement("OPTION");
                    option.value = item.value;
                    option.innerText = item.text;
                ...
                $('#id_estacion,#id_limite, #id_proyecto, #id_parametro').selectpicker('refresh');

            })
            .catch(error => console.log(error));
    }
    var obtenerEstacionesConProyecto = function () {
        var model = {
            fecha_inicio: fecha_inicio.format('YYYY-MM-DD'),
            fecha_fin: fecha_fin.format('YYYY-MM-DD'),
        ...
        $.post(urlObtenerEstaciones, model)
            .then(response => {
                $("#id_estacion, #id_parametro").empty();
                ...
                $('#id_estacion, #id_parametro').selectpicker('refresh');
