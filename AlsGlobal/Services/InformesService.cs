...
namespace AlsGlobal.Services
{
  public interface IInformesService
  {
    ...
    Task<ArchivoViewModel> GetDocumentoInformes(int[] id_documento);
  }
  public class InformesService : Service, IInformesService
  {
    private readonly HttpClient _httpClient;
    private readonly IAspNetUser _aspNetUser;
    private readonly IHttpContextAccessor _httpContext;
    public InformesService(HttpClient httpClient,
    ...
    public async Task<ArchivoViewModel> GetDocumentoInformes(int[] id_documento)
    {
     ...
     _httpClient.DefaultRequestHeaders.Add("culture", cultura);_httpClient.DefaultRequestHeaders.Add("Accept", "application/json");
      var response = await _httpClient.PostAsync("GetDocumentoInformes", contenido);
    ...
    public async Task<ResponseServiceViewModel<List<string>, List<string>, List<string>>> Obtener(FiltrosViewModel filtro, int page = 1, int rowPage=20)
    {
      ...
      _httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", _aspNetUser.ObtenerToken());
     _httpClient.DefaultRequestHeaders.Add("culture", cultura);_httpClient.DefaultRequestHeaders.Add("Accept", "application/json");
      var response = await _httpClient.PostAsync("GetInformes?page=" + page + "&rowPage=" + rowPage, contenido);
