class FilterDataTable {
    ...
    _agregarValorEnFiltro(span) {
        ...
        if (!this._filter.any(x => x.id === property)) {
            ...
            this._filter.firstOrDefault(x => x.id === property).condition = condition;
    ...
    _onDragOver(event, element){
        ...
        $(".drag__over__right").removeClass("drag__over__right");
        // $(".drag__over__left").removeClass("drag__over__left");


        var indexElement = $(element).index();
        
        this.intervalIndex = (element.offsetWidth / 2) > event.offsetX ? -1 : 0;

        $.each( this._props.element.find("tr"), (index, value) =>{
            // if(this.intervalIndex === 0)
                $(value.children[indexElement]).addClass("drag__over__right");
            // else 
            //     $(value.children[indexElement]).addClass("drag__over__left");
        });
    ...
    _onDrop(event, element){
        ...
        // indexDrag = indexDrag -  this.intervalIndex;
